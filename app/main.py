...
def initialize_session_state():
    ...
    if "initialized" not in st.session_state:
        ...
        st.session_state.selected_component = (
            list(COMPONENTS.keys())[0] if COMPONENTS else None
        )
        ...
        for component_data in COMPONENTS.values():
            for sensor_name in component_data["all_sensors"]:
                ...
                if sensor_name in SENSOR_METADATA:
                    st.session_state.historical_data[sensor_name] = deque(
                        maxlen=HISTORY_LENGTH
                    )
                    st.session_state.rul_predictions[sensor_name] = SENSOR_METADATA[
                        sensor_name
                    ]["initial_rul"]
                else:
                    st.warning(
                        f"Sensor '{sensor_name}' listed in COMPONENTS but not found in SENSOR_METADATA. Skipping initialization for this sensor."
                    )
...
def generate_single_sensor_value(sensor_name, condition_multiplier=1.0):
    ...
    current_rul = st.session_state.rul_predictions.get(
        sensor_name, metadata["initial_rul"]
    )
...
def update_all_sensor_data():
    ...
    for sensor_name, current_value in new_values.items():
        ...
        if sensor_name in st.session_state.historical_data:
            ...
            health_factor = {"Good": 1.0, "Degrading": 1.5, "Critical": 3.0}.get(
                health_status, 1.0
            )

            total_degrade = base_degrade * condition_factor * health_factor
            st.session_state.rul_predictions[sensor_name] = max(
                0, st.session_state.rul_predictions[sensor_name] - total_degrade
            )

            # Update health and anomaly status
            st.session_state.health_statuses[sensor_name] = health_status
            st.session_state.anomaly_statuses[sensor_name] = detect_anomaly(
...
def get_health_status(sensor_name, current_value):
    ...
    if value_type == "wear_inverse":
        ...
        )  # 10% above min is critical
        degrading_threshold = (
            min_val + (max_val - min_val) * 0.3
        )  # 30% above min is degrading
        ...
        if (
            current_value < critical_band_outer_lower
            or current_value > critical_band_outer_upper
        ):
            return "Critical"
        elif (
            current_value < warning_band_outer_lower
            or current_value > warning_band_outer_upper
        ):
            return "Degrading"
        else:
            return "Good"
...
def detect_anomaly(sensor_name, current_value):
    ...
    statistical_anomaly = (
        z_score > 2.5
    )  # Value is 2.5 standard deviations away from mean
    ...
    if len(history) >= 10:
        ...
        trend_anomaly = trend_change > (range_span * 0.05) and trend_change > (
            std_dev * 1.5
        )
    else:
        trend_anomaly = False
...
def generate_alerts(sensor_name, current_value, health_status):
    ...
    for alert in st.session_state.alerts:
        if (
            alert["sensor"] == sensor_name
            and alert["severity"] == health_status
            and (datetime.now() - alert["timestamp"]) < timedelta(minutes=5)
        ):  # Avoid re-alerting too quickly
            return
    ...
    if health_status == "Critical":
        alert = {
            ...
            "recommendation": get_maintenance_recommendation(
                sensor_name, health_status
            ),
        }
        st.session_state.alerts.append(alert)
    elif (
        health_status == "Degrading" and np.random.random() < 0.15
    ):  # Reduced chance for degrading alerts
        alert = {
            "timestamp": datetime.now(),
            "sensor": sensor_name,
            "severity": "Warning",
...
def get_maintenance_recommendation(sensor_name, health_status):
    ...
    return recommendations.get(sensor_name, default_rec).get(
        health_status, "Monitor closely for changes."
    )
...
def update_component_health_scores():
    ...
    for component_name, component_data in COMPONENTS.items():
        ...
        for sensor_name in component_data["all_sensors"]:
            ...
            if sensor_name in SENSOR_METADATA:
                health_status = st.session_state.health_statuses.get(
                    sensor_name, "Good"
                )
                score = {"Good": 100, "Degrading": 60, "Critical": 20}.get(
                    health_status, 100
                )
                ...
                rul_factor = max(
                    0.0, current_rul / initial_rul
                )  # RUL factor goes from 0 to 1
                ...
                priority_factor = {
                    ...
                    "medium": 1.0,
                    "low": 0.8,
                }.get(SENSOR_METADATA[sensor_name].get("priority", "medium"), 1.0)
        ...
        if health_scores:
            ...
            total_priority_weight = sum(
                [
                    {"critical": 2.0, "high": 1.5, "medium": 1.0, "low": 0.8}.get(
                        SENSOR_METADATA[s].get("priority", "medium"), 1.0
                    )
                    for s in component_data["all_sensors"]
                    if s in SENSOR_METADATA
                ]
            )

            # Ensure division by zero is avoided if total_priority_weight is 0
            normalized_score = (
                (sum(health_scores) / total_priority_weight) * (100 / 100)
                if total_priority_weight > 0
                else 100
            )
            st.session_state.component_health_scores[component_name] = max(
                0, min(100, normalized_score)
            )
        else:
            st.session_state.component_health_scores[component_name] = (
                100  # Default if no sensors
            )
...
def update_performance_metrics():
    ...
    st.session_state.performance_metrics = {
        ...
        "Critical Sensors": len(
            [s for s in st.session_state.health_statuses.values() if s == "Critical"]
        ),
        "Degrading Sensors": len(
            [s for s in st.session_state.health_statuses.values() if s == "Degrading"]
        ),
        "Anomalies Detected": len(
            [a for a in st.session_state.anomaly_statuses.values() if a]
        ),
        "Operating Hours": st.session_state.total_operating_hours,
        "Active Alerts": len(
            [a for a in st.session_state.alerts if a["severity"] == "Critical"]
        ),
    }
...
def display_hierarchical_sensor_monitoring(selected_component):
    ...
    valid_key_sensors = [
        s for s in component_data["key_sensors"] if s in SENSOR_METADATA
    ]
    ...
    valid_all_sensors = [
        s for s in component_data["all_sensors"] if s in SENSOR_METADATA
    ]
...
def display_component_overview(component_name, component_data):
    ...
    with col1:
        ...
        st.markdown(
            f"<h3 style='color:{health_color}'>{health_score:.1f}%</h3>",
...
def display_key_sensor_analysis(sensor_name):
    ...
    with col1:
        st.metric(
            ...
            value=f"{current_value} {metadata['unit']}"
        ...
        health_color = (
            ...
            else ("orange" if health_status == "Degrading" else "red")
        )
        st.markdown(
            f"**Health Status:** <span style='color:{health_color}'>**{health_status}**</span>",
            unsafe_allow_html=True,
        )
    ...
    with col2:
        ...
        st.markdown(
            f"**Anomaly Detected:** <span style='color:{anomaly_color}'>**{'Yes' if is_anomaly else 'No'}**</span>",
            unsafe_allow_html=True,
        )
    ...
    with col3:
        ...
        if history_data:
            ...
            st.plotly_chart(
                ...
                key=f"trend_chart_{sensor_name}_{st.session_state.last_update_time.timestamp()}",
            )
        else:
...
def display_parameter_category_analysis(
    sensor_list, component_name
):  # Added component_name for keying
    ...
    sorted_categories = sorted(
        sensors_by_category.keys(),
        key=lambda x: list(PARAMETER_CATEGORIES.keys()).index(x)
        if x in PARAMETER_CATEGORIES
        else len(PARAMETER_CATEGORIES),
    )
    ...
    for category in sorted_categories:
        ...
        for i in range(0, len(sensors_by_category[category]), cols_per_row):
            ...
            for j, sensor_name in enumerate(
                sensors_by_category[category][i : i + cols_per_row]
            ):
                with cols[j]:
                    metadata = SENSOR_METADATA.get(sensor_name)
                    if not metadata:
                    ...
                    current_value = st.session_state.current_sensor_values.get(
                        sensor_name, "N/A"
                    )
                    health_status = st.session_state.health_statuses.get(
                        sensor_name, "Unknown"
                    ...
                    st.markdown(f"**{sensor_name}**")
                    st.markdown(
                        f"<h4 style='color:{health_color}'>{current_value} {metadata['unit']}</h4>",
                        unsafe_allow_html=True,
                    )
                    st.markdown(
                        f"Status: <span style='color:{health_color}'>**{health_status}**</span>",
                        unsafe_allow_html=True,
                    )

                    # Mini-trend graph for each sensor
                    history_data = list(
                        st.session_state.historical_data.get(sensor_name, deque())
                    )
                    if history_data:
                        df_mini_trend = pd.DataFrame(
                            {"Time": range(len(history_data)), "Value": history_data}
                        )
                        ...
                        st.plotly_chart(
                            ...
                            key=f"mini_trend_{component_name}_{category}_{sensor_name}_{st.session_state.last_update_time.timestamp()}",
                        )
                    else:
                        st.text("No data.")
...
st.session_state.fault_injection = st.sidebar.checkbox(
    "Enable Fault Injection (for demo)",
    value=st.session_state.fault_injection,
    key="fault_injection_checkbox",
)
...
if st.session_state.selected_component in COMPONENTS:
    default_component_index = list(COMPONENTS.keys()).index(
        st.session_state.selected_component
    )
...
# Load HOST_URL from environment variable, fallback to localhost
HOST_URL = os.getenv("HOST_URL", "http://localhost")

# Dashboard URLs
DASHBOARD_URLS = {
    "🛑 Braking System": f"{HOST_URL}:8502",
    "🔧 Engine System": f"{HOST_URL}:8503",
    "🛞 Tire System": f"{HOST_URL}:8504",
    "📊 Vibration Analysis": f"{HOST_URL}:8505",
}

# Create navigation buttons using st.link_button (opens in new tab)
for dashboard_name, url in DASHBOARD_URLS.items():
    st.sidebar.link_button(dashboard_name, url, use_container_width=True)
...
if (
    st.session_state.auto_update_enabled
    and (current_time - st.session_state.last_update_time).total_seconds()
    >= UPDATE_INTERVAL_SECONDS
):
...
with col_overall1:
    overall_health_score = st.session_state.performance_metrics.get(
        "Overall Health", 100
    )
    overall_health_color = (
        "green"
        if overall_health_score >= 80
        else ("orange" if overall_health_score >= 50 else "red")
    )
    st.markdown(f"**Overall Fleet Health**")
    st.markdown(
        f"<h3 style='color:{overall_health_color}'>{overall_health_score:.1f}%</h3>",
        unsafe_allow_html=True,
    )
...
with col_overall2:
    st.metric(
        ...
        f"{st.session_state.performance_metrics.get('Operating Hours', 0):.1f} hrs",
    )
with col_overall3:
    st.metric(
        "Critical Alerts", st.session_state.performance_metrics.get("Active Alerts", 0)
    )
with col_overall4:
    st.metric(
        "Total Anomalies",
        st.session_state.performance_metrics.get("Anomalies Detected", 0),
    )
with col_overall5:
...
if not st.session_state.auto_update_enabled:
    st.info(
        "Auto-update is disabled. Please enable it from the sidebar to start real-time monitoring."
    )
